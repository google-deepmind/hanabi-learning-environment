...
class ChenyangAgent(Agent):
    ...
    @staticmethod
    def playable_card(card_index, observation, fireworks):
        possible_card = ChenyangAgent.get_possible(card_index, observation)
        all_playable_card = ChenyangAgent.all_playable_cards(fireworks)
        return set(possible_card).issubset(set(all_playable_card))

    @staticmethod
    def discardble_card(card_index, observation, fireworks):
        possible_card = ChenyangAgent.get_possible(card_index, observation)
        all_discardble_card = ChenyangAgent.all_discardble_cards(fireworks)
        return set(possible_card).issubset(set(all_discardble_card))
    ...
    def act(self, observation):
        ...
        if observation['information_tokens'] > 0:
            ...
            for card_index, card in enumerate(you_hand):
                if ChenyangAgent.playable_card(card_index, observation, fireworks) and card['rank'] is None:
                    return {
